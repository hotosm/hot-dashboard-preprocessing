...
// This class is the processing the data for the DummyProject project
class DummyProject extends _AbstractProject2.default {
  ...
  getNbWorkshops(data) {
    ...
    for (; yearMax >= yearMin; yearMax--) {
      ...
      for (; month >= 0; month--) {
        let nbWorkshopsFiltered = data.capacitybuilding.nbworkshops.data.filter(row => row[endDate] && new Date(row[endDate]).getFullYear() === yearMax && new Date(row[endDate]).getMonth() === month);
