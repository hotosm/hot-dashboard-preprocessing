...
class PreProcessor {
      ...
      reader
        .getCsv(
          CONFIG.googleEndPoint +
            CONFIG.projects[0] +
            CONFIG.googleSheetEndUrl +
            CONFIG.projects[1],
          this.getDataFromProjectsFile
        )
        .then(allProjects => {
  ...
  getDataFromProjects(projectSource, i) {
    return new Promise((resolve, reject) => {
      reader
        .getCsv(
          CONFIG.googleEndPoint +
            this.getFileKey(projectSource[i][projectConfigUrl]) +
            CONFIG.googleSheetEndUrl +
            projectSource[i][sheetName],
          this.getAllDatas
        )
  ...
  async getAllDatas(result) {
    ...
    console.log('getAllDatas: ', allDatasFromAPIwithLinks);
    const generalData = {
      mapping: {},
      capacitybuilding: {},
      awareness: {},
      ...
      if (allDatasFromAPIwithLinks[i][url] !== undefined) {
        ...
        switch (allDatasFromAPIwithLinks[i].type.toLowerCase()) {
          ...
          case 'api':
            break;
          case 'json':
            dataGeneratedWithLink.data = await reader.getJson(
              allDatasFromAPIwithLinks[i][url],
              allDatasFromAPIwithLinks[i].name
            );
            break;
          // If there is a csv file
          case 'csv':
            dataGeneratedWithLink.data = await reader.getCsv(
              allDatasFromAPIwithLinks[i][url],
              result => result.data
            );
            break;
          ...
          case 'google sheet':
            dataGeneratedWithLink.data = await reader.getCsv(
              CONFIG.googleEndPoint +
                this.getFileKey(allDatasFromAPIwithLinks[i][url]) +
                CONFIG.googleSheetEndUrl +
                allDatasFromAPIwithLinks[i][sheetName],
              result => result.data
            );
            break;
          default:
