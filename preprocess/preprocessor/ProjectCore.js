...
class ProjectCore extends AbstractProject {
  ...
  getNbSubwards(data) {
    ...
    if (data.mapping.hasOwnProperty('nbSubwardsCompleted')) {
      const nbSubwardsFromData = data.mapping.nbSubwardsCompleted.data.filter(
        row => row['Ward'] === 'TOTAL'
      )[0];
      ...
      for (let k = 3; k < Object.keys(nbSubwardsFromData).length; k++) {
        let exist = false;
        const divisionData = Object.values(nbSubwardsFromData)[k];
        if (divisionData > 0) {
          const nbElements = Object.keys(nbSubwardsFromData)[k].split(' ').length;
          ...
          const subwardType = Object.keys(nbSubwardsFromData)[k].substring(
            0,
            Object.keys(nbSubwardsFromData)[k].length - 10
          );
          const month = Object.keys(nbSubwardsFromData)[k].split(' ', nbElements)[nbElements - 2];
          const year = Object.keys(nbSubwardsFromData)[k].split(' ', nbElements)[nbElements - 1];
          const divisionDate = new Date(month + ' 1 ' + year);
          for (let l = 0; l < nbSubwards.length && !exist; l++) {
            ...
            if (
              ...
              const divisionDataTemp = {
                extend: subwardType,
                label:
                  divisionDate.toUTCString().split(' ', 3)[2] +
                  ' ' +
                  divisionDate.toUTCString().split(' ', 4)[3],
                date: divisionDate,
                value: divisionData
              };
              let res = [];
  ...
  getNbAttendeesMonthly(data) {
    ...
    if (data.capacitybuilding.hasOwnProperty('nbattendeesmonthly')) {
      const attendees = data.capacitybuilding.nbattendeesmonthly.data;
      // This array will hold the final data
      let attendeesArray = [];
      // This variable will be used when data is inserted in the array
      let exist = false;
      for (let i = 0; i < attendees.length; i++) {
        const date = new Date(attendees[i][endDate]);
        // This loop is here to add the row in the right array cell in order to have a descending order
        for (let j = 0; j < attendeesArray.length && !exist; j++) {
          ...
          if (
            date.getFullYear() > attendeesArray[j].date.getFullYear() ||
            (date.getMonth() > attendeesArray[j].date.getMonth() &&
              attendeesArray[j].date.getFullYear() === date.getFullYear())
          ) {
            ...
            attendeesTemp.push({
              date,
              label:
                date.toUTCString().split(' ', 3)[2] + ' ' + date.toUTCString().split(' ', 4)[3],
          ...
          // If the date of the current row is equal to the date of the item in the array (month and year because it's filtered this way)
            ...
            attendeesArray[j].date.getMonth() === date.getMonth() &&
            attendeesArray[j].date.getFullYear() === date.getFullYear()
          ) {
  ...
  getNbAttendeesInstitutions(data) {
    ...
    if (data.capacitybuilding.hasOwnProperty('nbattendeesinstitutions')) {
      const nbAttendeesInstitutionsFromData = data.capacitybuilding.nbattendeesinstitutions;
      ...
      for (let i = 0; i < nbAttendeesInstitutionsFromData.data.length; i++) {
        for (let j = 0; j < nbAttendeesInstitutions.data.length; j++) {
          nbAttendeesInstitutions.data[j].value +=
            nbAttendeesInstitutionsFromData.data[i][nbAttendeesInstitutions.data[j].extend];
        }
      }
      // We store the data calculated in the global data
  ...
  getNbAttendeesTraining(data) {
    ...
    if (data.capacitybuilding.hasOwnProperty('nbattendeestraining')) {
      const nbAttendeesTrainingFromData = data.capacitybuilding.nbattendeestraining.data;
  ...
  getNbWorkshopsMonthly(data) {
    ...
    if (data.capacitybuilding.hasOwnProperty('nbworkshopsmonthly')) {
      data.capacitybuilding.nbworkshopsmonthly.data.filter(function(row) {
        const rowYear = new Date(row[endDate]).getFullYear();
        if (rowYear < yearMin) {
          yearMin = rowYear;
        }
      });
      ...
      for (; yearMax >= yearMin; yearMax--) {
        ...
        for (; month >= 0; month--) {
          const nbWorkshopsFiltered = data.capacitybuilding.nbworkshopsmonthly.data.filter(
            row =>
              row[endDate] &&
              new Date(row[endDate]).getFullYear() === yearMax &&
              new Date(row[endDate]).getMonth() === month
          );
          if (nbWorkshopsFiltered.length > 0) {
            nbWorkshops[counter] = {
              ...
              label:
                new Date(nbWorkshopsFiltered[0][endDate]).toUTCString().split(' ', 3)[2] +
                ' ' +
                new Date(nbWorkshopsFiltered[0][endDate]).toUTCString().split(' ', 4)[3]
            };
            counter++;
          }
        }
      }
      const nbWorkshopsStored = {
  ...
  getNbParticipantsGender(data) {
    ...
    if (data.community.hasOwnProperty('nbparticipantsgender')) {
      const nbParticipantsFromData = data.community.nbparticipantsgender.data;
      // This array will hold the final data
      let nbParticipants = [];
      // This variable will be used when data is inserted in the array
      let exist = false;
      for (let i = 0; i < nbParticipantsFromData.length; i++) {
        ...
        if (nbParticipantsFromData[i].Number > 0) {
          ...
          for (let j = 0; j < nbParticipants.length && !exist; j++) {
            // If the date of the current row is greater (newer) than the item in the array (year greater or same yeah but month greater)
            if (
              date.getFullYear() > nbParticipants[j].date.getFullYear() ||
              (date.getMonth() > nbParticipants[j].date.getMonth() && date.getFullYear()) ===
                nbParticipants[j].date.getFullYear()
            ) {
              // This array will store the data to be added in the middle of the old one
              const participantTemp = [];
              participantTemp.push({
                date: date,
                label:
                  date.toUTCString().split(' ', 3)[2] + ' ' + date.toUTCString().split(' ', 4)[3],
                female:
                  nbParticipantsFromData[i].Female === '' ? 0 : nbParticipantsFromData[i].Female,
            ...
            // If the date of the current row is equal to the date of the item in the array (month and year because it's filtered this way)
              ...
              nbParticipants[j].date.getMonth() === date.getMonth() &&
              nbParticipants[j].date.getFullYear() === date.getFullYear()
            ) {
  ...
  getNbParticipantsNew(data) {
    ...
    if (data.community.hasOwnProperty('nbparticipantsnew')) {
      const nbParticipantsFromData = data.community.nbparticipantsnew.data;
      // This array will hold the final data
      let nbParticipants = [];
      // This variable will be used when data is inserted in the array
      let exist = false;
      for (let i = 0; i < nbParticipantsFromData.length; i++) {
        const date = new Date(nbParticipantsFromData[i].Date);
        if (nbParticipantsFromData[i].Number > 0) {
          // This loop is here to add the row in the right array cell in order to have a descending order
          for (let j = 0; j < nbParticipants.length && !exist; j++) {
            // If the date of the current row is greater (newer) than the item in the array (year greater or same yeah but month greater)
  ...
  getNbParticipantsType(data) {
    ...
    if (data.community.hasOwnProperty('nbparticipantstype')) {
      const nbParticipantsFromData = data.community.nbparticipantstype.data;
      // This array will hold the final data
